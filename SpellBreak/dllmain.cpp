...
#define OFFSET_UWORLD 0x63B6528 //48 8B 1D ?? ?? ?? ?? 48 85 DB 74 3B 41
...
void Menu()
{
	...
	if (menu)
	{
		ImGui::Begin(_xor_("SpellBreak Private").c_str(), &menu, ImVec2(420, 165), 1.0f, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
		ImGui::Checkbox(_xor_("Distance Esp").c_str(), &Esp);
...
void updateaddr()
{
	hWnd = FindWindow(0, _xor_(L"SpellBreak  ").c_str());
	...
	game_instance = ReadMemory<uint64_t>(uworld + 0x188);
...
void initialize()
{
	...
	w2sAddress = (DWORD_PTR)FindPattern((BYTE*)base_address, 0x10000000, (BYTE*)_xor_("\xE9\x00\x00\x00\x00\x55\x48\x83\xEC\x20\x48\x8D\x6C\x24\x20\x48\x89\x4D\x10\x48\x89\x55\x18\x48\x8B\x05\x9A\xBE\xA9\xFC").c_str(), _xor_("xxxxx????xxxxxxxxxxx").c_str(), 0);
...
//	GetObjectNameAddr = (DWORD_PTR)FindPattern((BYTE*)base_address, 0x10000000, (BYTE*)_xor_("").c_str(), _xor_("xxxxxxxxxxxxxxxxxxxxxxxxxxx????xxxxxxxxxxxxxxxxxx????xxxxxx????xx????x????xxxxxxxxxxxxx").c_str(), 0);
	...
	auto btAddrUWorld = FindPattern((PBYTE)base_address, 0x10000000, (PBYTE)_xor_("\x48\x8B\x1D\x00\x00\x00\x00\x48\x85\xDB\x74\x3B\x41").c_str(), _xor_("xxx????xxxxxx").c_str(), 0);
...
//	std::printf(_xor_("UworldAddress: %p.\n").c_str(), UworldAddress - base_address);
	...
	DiscordGetAsyncKeyStatePattern = FindPattern((BYTE*)DiscordBase, 0x268000, (BYTE*)"\xE8\x00\x00\x00\x00\x41\x88\x06\x48\x83\xC4\x30", _xor_("x????xxxxxxx").c_str(), 0);
...
VOID Main() {
	...
	HWND window = FindWindow(0, _xor_(L"SpellBreak  ").c_str());
